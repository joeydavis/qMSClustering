...
def heatmap(x, row_header, column_header, row_method,
    ...
    [axcb_x, axcb_y, axcb_w, axcb_h] = [0.9,0.22,0.05,0.6]
    ...
    for i in range(x.shape[0]):
        if row_method != None:
            if len(row_header)<100: ### Don't visualize gene associations when more than 100 rows
                axm.text(x.shape[1]-0.5, i, '  '+row_header[idx1[i]], verticalalignment="center", fontsize=math.ceil(36*12/x.shape[0]))
            new_row_header.append(row_header[idx1[i]])
        else:
            if len(row_header)<100: ### Don't visualize gene associations when more than 100 rows
                axm.text(x.shape[1]-0.5, i, '  '+row_header[i], verticalalignment="center", fontsize=math.ceil(36*12/x.shape[0])) ### When not clustering rows
            new_row_header.append(row_header[i])
    for i in range(x.shape[1]):
        if column_method != None:
            axm.text(i, -0.9, ' '+column_header[idx2[i]], rotation=270, verticalalignment="top", fontsize=math.ceil(18*12/x.shape[1])) # rotation could also be degrees
            new_column_header.append(column_header[idx2[i]])
        else: ### When not clustering columns
            axm.text(i, -0.9, ' '+column_header[i], rotation=270, verticalalignment="top", horizontalalignment="center", fontsize=math.ceil(18*12/x.shape[1]))
    ...
    axcb.set_title("colorkey", fontsize = 12)
